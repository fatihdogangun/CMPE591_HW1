...
class CNN(nn.Module):
    ...
    def forward(self, image, action):
        ...
        cnn_features = self.cnn(image).view(batch_size, -1) 
...
def train(model = CNN(), train_dataloader = train_dataloader, validation_dataloader = validation_dataloader, num_epochs=600, lr=1e-3, device=None):
    ...
    for epoch in range(num_epochs):
        ...
        if validation_epoch_loss < best_loss:
            ...
            torch.save(model.state_dict(), os.path.join("save", "hw1_2.pth"))
...
def test(test_dataloader = test_dataloader, device=None):
    ...
    model.load_state_dict(torch.load(os.path.join("save", "hw1_2.pth")))
